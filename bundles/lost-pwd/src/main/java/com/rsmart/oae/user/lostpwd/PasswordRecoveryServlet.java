...
@ServiceDocumentation(
    bindings = {
        @ServiceBinding(
            ...
            bindings = {"rsmart/lostPassword"},
            selectors = { @ServiceSelector(name = "recover") }
        )
    },
    methods = {
        @ServiceMethod(
            ...
            description = {
                ...
                "The guid passed in must match the one in the guid property of the rsmart/lostPassword node."
            },
            response = {
...
public class PasswordRecoveryServlet extends SlingAllMethodsServlet {
  ...
  protected void processRequest(SlingHttpServletRequest request, SlingHttpServletResponse response)
    ...
    if ("recover".equals(selector)) {
      try {
        ...
        String redirect = lostPasswordService.checkPasswordRetrieval(request, guid, passwordRecovery);
      ...
      User user = lostPasswordService.changePassword(session, guid, passwordRecovery, newPassword);
   ...
   * 
   * checkPwd:
   *    first, call checkPasswordRetrieval to see if the guid matches and it hasn't already been called
   *    if no exception, redirect the user to the home screen that will then pop-up a dialog to change
   *    the password (with the guid and recoveryFile path as hidden params)
   * 
   * changePwd:
   *    call changePassword... return json success or fail
   * 
   */
